...
class VoiceAssistant:
    def __init__(self):
        ...
        # --- Buffer for continuous input ---
        self.speech_buffer = ""
        self.last_buffer_time = time.time()

        # Queue for TTS outputs
    ...
    async def buffer_flush_loop(self):
        while True:
            ...
            if len(words) >= BUFFER_MAX_WORDS or time_since_last >= BUFFER_MAX_SECONDS:
    ...
    async def stream_to_tts(self, text):
        ...
        self.stop_tts_flag = False
        try:
            connection = deepgram.speak.v1.connect(model="aura-asteria-en",
                                                   encoding="linear16",
                                                   sample_rate=str(SAMPLE_RATE))
            audio_chunks = []
...
async def main():
    ...
    # Start TTS worker and buffer flush loop
